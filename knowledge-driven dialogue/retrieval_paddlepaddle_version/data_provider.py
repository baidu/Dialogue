...
def prepare_batch_data(insts,
    ...
    context_id, next_sent_context_index, context_attn_bias = pad_batch_data(
        batch_context_ids, pad_idx=0, max_len=max_len, return_next_sent_pos=True, return_attn_bias=True)
    ...
    for i in range(len(insts)): 
        out = [context_id[i], context_pos_id[i], context_segment_id[i], context_attn_bias[i], kn_id[i], labels_list[i], next_sent_context_index[i]]
        out_list.append(out)
    return out_list
...
class DataProcessor(object):
    ...
    def __init__(self, data_dir, vocab_path, max_seq_len, do_lower_case):
    ...
    def _read_data(cls, input_file):
        ...
        with open(input_file, "r") as f:
            ...
            for line in f:
                line = line.rstrip('\n')
                out_list = parser.parser_out(line)
        ...
        def instance_reader():
            for epoch_index in range(epoch):
                ...
                for (index, example) in enumerate(examples):
                    ...
                    instance = [feature.context_ids, feature.context_pos_ids, feature.segment_ids, feature.label_ids, feature.kn_ids]
                    yield instance
...
class MatchProcessor(DataProcessor):
    ...
    def _create_examples(self, lines, set_type):
        ...
        for (i, line) in enumerate(lines):
            ...
            examples.append(
                InputExample(
                    guid=guid, context_text=context_text, response_text=response_text, kn_text=kn_text, label_text=label_text))
        return examples
...
def convert_single_example(ex_index, example, label_list, max_seq_length):
    ...
    if len(tokens_context) > max_seq_length - len(tokens_response) - 3: 
        tokens_context = tokens_context[len(tokens_context) + len(tokens_response) - max_seq_length + 3:]
