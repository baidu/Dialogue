...
from transformer_model import encoder, pre_process_layer
...
class RetrievalModel(object):
    def __init__(self,
        ...
        self._build_model(context_ids, context_pos_ids, context_segment_ids, context_attn_mask, kn_ids)
    ...
    def _build_model(self, context_ids, context_pos_ids, context_segment_ids, context_attn_mask, kn_ids): 
        ...
        self.memory_encoder_out, self.memory_encoder_proj_out = self._build_memory_network(kn_ids)

    def get_context_response_memory(self, context_next_sent_index): 
        context_out = self.get_pooled_output(context_next_sent_index)
        kn_context = self.gru_decoder_with_attention(context_out, self.memory_encoder_out, self.memory_encoder_proj_out)
    ...
    def gru_decoder_with_attention(self, context_out, encoder_vec, encoder_vec_proj): 
        """ 
        rnn = fluid.layers.DynamicRNN()
        with rnn.block(): 
            current_context = rnn.step_input(trg_emb)
            encoder_vec = rnn.static_input(encoder_vec)
            encoder_vec_proj = rnn.static_input(encoder_vec_proj)
            hidden_mem = rnn.memory(init=context_out, need_reorder=True)
            context = self.attention(hidden_mem, encoder_vec, encoder_vec_proj)
            rnn.update_memory(hidden_mem, context)
            rnn.output(context)
        #last_step = fluid.layers.sequence_last_step(rnn())
        #return last_step
        """
        context = self.attention(context_out, encoder_vec, encoder_vec_proj)
        return context
